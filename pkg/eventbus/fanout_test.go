...
func TestFanOutEventBus_AddSpokeReplaysSubscriptions(t *testing.T) {
	...
	late := newStubEventBus()
	late.subscribeFunc = func(pattern string, _ EventHandler) (Subscription, error) {
		late.mu.Lock()
		defer late.mu.Unlock()
		subscribedPatterns = append(subscribedPatterns, pattern)
		return &stubSubscription{}, nil
	}

	if err := fan.AddSpoke(NamedEventBus{Name: "late", Bus: late}); err != nil {
		t.Fatalf("unexpected AddSpoke error: %v", err)
	}

	late.mu.Lock()
	defer late.mu.Unlock()
	if len(subscribedPatterns) != 2 {
		t.Fatalf("expected 2 replayed subscriptions, got %d", len(subscribedPatterns))
	}
	if subscribedPatterns[0] != "events.>" || subscribedPatterns[1] != "commands.*" {
		t.Errorf("unexpected patterns: %v", subscribedPatterns)
	}
}

func TestFanOutEventBus_ReplaceSpokeReplaysSubscriptions(t *testing.T) {
	b1 := newStubEventBus()
	...
	var subscribedPatterns []string
	replacement := newStubEventBus()
	replacement.subscribeFunc = func(pattern string, _ EventHandler) (Subscription, error) {
		replacement.mu.Lock()
	...
	if err := fan.ReplaceSpoke("b1", NamedEventBus{Name: "b1", Bus: replacement}); err != nil {
		t.Fatalf("unexpected ReplaceSpoke error: %v", err)
	}

	replacement.mu.Lock()
	defer replacement.mu.Unlock()
	if len(subscribedPatterns) != 1 {
		t.Fatalf("expected 1 replayed subscription, got %d", len(subscribedPatterns))
	}
	if subscribedPatterns[0] != "events.>" {
		t.Errorf("unexpected pattern: %s", subscribedPatterns[0])
	}

	b1.mu.Lock()
	defer b1.mu.Unlock()
	if !b1.closed {
		t.Error("old spoke should have been closed")
	}
}
